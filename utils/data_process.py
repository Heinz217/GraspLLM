...
def generate_notestlink(dataset):
    ...
    data = torch.load(f"dataset/{dataset}/processed_data.pt", weights_only=False)
    print(data)
    ...
    link_test_path = os.path.join(data_dir, "ocs_test.jsonl")
...
def generate_multi_hop_x_arxiv_notestlink(emb="sbert"):
    data = torch.load(fos.path.join(dataset_dir("arxiv"), "processed_data_link_notest.pt"), weights_only=False)
    x = torch.load(fos.path.join(dataset_dir("arxiv"), "{emb}_x.pt"), weights_only=False)
    ...
    link_test_path = os.path.join(fdataset_dir("arxiv"), "ocs_test.jsonl")
    with open(link_test_path, 'r') as f:
        link_test_lines = f.readlines()
    link_test_lines = [json.loads(line) for line in link_test_lines]
    n = data.num_nodes
    ...
    for i in range(4):
        ...
        torch.save(x[mask].cpu(), fos.path.join(dataset_dir("arxiv"), "{emb}_{i + 1}hop_x_notestlink.pt"))



def generate_multi_hop_x_products_notestlink(emb="sbert"):
    print(emb)
    data = torch.load(fos.path.join(dataset_dir("products"), "processed_data_link_notest.pt"), weights_only=False)
    x = torch.load(fos.path.join(dataset_dir("products"), "{emb}_x.pt"), weights_only=False)
    row, col = data.edge_index
    deg = degree(col, x.size(0), dtype=x.dtype)
    deg_inv_sqrt = deg.pow(-0.5)
    deg_inv_sqrt[deg_inv_sqrt == float('inf')] = 0
    norm = deg_inv_sqrt[row] * deg_inv_sqrt[col]
    link_test_path = os.path.join(fdataset_dir("products"), "ocs_test.jsonl")
    with open(link_test_path, 'r') as f:
        link_test_lines = f.readlines()
    link_test_lines = [json.loads(line) for line in link_test_lines]
    n = data.num_nodes
    ...
    for i in range(4):
        ...
        torch.save(x[mask].cpu(), fos.path.join(dataset_dir("products"), "{emb}_{i + 1}hop_x_notestlink.pt"))


def generate_multi_hop_x(dataset, emb="sbert"):
    data = torch.load(f"dataset/{dataset}/processed_data.pt", weights_only=False)
    x = torch.load(f"dataset/{dataset}/{emb}_x.pt", weights_only=False)
    row, col = data.edge_index
    deg = degree(col, x.size(0), dtype=x.dtype)
    deg_inv_sqrt = deg.pow(-0.5)
    deg_inv_sqrt[deg_inv_sqrt == float('inf')] = 0
