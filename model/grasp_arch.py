...
class Vicuna2LayerSHAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.hidden_dim = getattr(config, "hidden_size", 4096)
        self.linear1 = nn.Linear(config.mm_hidden_size, self.hidden_dim)
        self.gelu = nn.GELU()
        self.linear2 = nn.Linear(self.hidden_dim, self.hidden_dim)
        self.target_seq_len = 5
        self.attention_proj = nn.Linear(self.hidden_dim, 1)

        self.query_vectors = nn.Parameter(torch.randn(self.target_seq_len, self.hidden_dim))
        nn.init.orthogonal_(self.query_vectors)
    ...
    def forward(self, x):
        ...
        for i in range(self.target_seq_len):
            query = self.query_vectors[i].unsqueeze(0).unsqueeze(0)  
            query = query.expand(batch_size, 1, -1) 
            
            attn_scores = torch.bmm(x, query.transpose(1, 2)) 
            attn_scores = attn_scores.squeeze(-1)

            scale_factor = 1.0 / math.sqrt(self.hidden_dim)
            attn_scores = attn_scores * scale_factor / self.temperature
            
            attn_weights = F.softmax(attn_scores, dim=1).unsqueeze(-1) 
            
            context = torch.sum(x * attn_weights, dim=1) 
            aggregated_tokens.append(context)
        
        output = torch.stack(aggregated_tokens, dim=1)
        
        if torch.isnan(output).any() or torch.isinf(output).any():
            print("[DEBUG] Output x has NaN or Inf!")
            
        return output


class Vicuna2LayerMHAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        # Read LLM hidden size from config instead of hard-coding 4096.
        self.hidden_dim = getattr(config, "hidden_size", 4096)
        self.linear1 = nn.Linear(config.mm_hidden_size, self.hidden_dim)
        self.gelu = nn.GELU()
        self.linear2 = nn.Linear(self.hidden_dim, self.hidden_dim)
        ...
        assert self.hidden_dim % self.num_heads == 0, (
            f"hidden_dim={self.hidden_dim} not divisible by num_heads={self.num_heads}")
        self.head_dim = self.hidden_dim // self.num_heads

        self.temperature = nn.Parameter(torch.ones(1))

        self.query_vectors = nn.Parameter(torch.randn(self.target_seq_len, self.num_heads, self.head_dim))
        for i in range(self.target_seq_len):
            nn.init.orthogonal_(self.query_vectors[i])
    ...
    def forward(self, x):
        ...
        query = self.query_vectors.unsqueeze(0).expand(batch_size, -1, -1, -1)
        query = query.permute(0, 2, 1, 3)
        
        attn_scores = torch.matmul(query, x.transpose(-2, -1))
        ...
        output = context.reshape(batch_size, self.target_seq_len, self.hidden_dim)
...
class Vicuna2LayerAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        # Read LLM hidden size from config.
        self.hidden_dim = getattr(config, "hidden_size", 4096)
        self.linear1 = nn.Linear(config.mm_hidden_size, self.hidden_dim)
        self.gelu = nn.GELU()
        self.linear2 = nn.Linear(self.hidden_dim, self.hidden_dim)
...
class Vicuna2LayerProjector(nn.Module):
    def __init__(self, config):
        super().__init__()
        # Read LLM hidden size from config.
        self.hidden_dim = getattr(config, "hidden_size", 4096)
        ...
        self.gelu = nn.GELU()
        self.linear2 = nn.Linear(self.hidden_dim, self.hidden_dim)
        self.pool = nn.AdaptiveAvgPool1d(output_size=5)
...
def build_graph_projector(config, delay_load=False, **kwargs):
    ...
    hidden_dim = getattr(config, 'word_embed_proj_dim', getattr(config, 'hidden_size', None))

    if projector_type == 'linear':
        return nn.Linear(config.mm_hidden_size, hidden_dim)

    elif projector_type == 'vicuna_2layer':
        projector = Vicuna2LayerProjector(config)
        return projector
    elif projector_type == 'vicuna_2layermh':
        projector = Vicuna2LayerMHAttention(config)
        return projector
    elif projector_type == 'vicuna_2layersh':
        projector = Vicuna2LayerSHAttention(config)
        return projector
...
class GraspMetaModel:

    def __init__(self, config):
        super(GraspMetaModel, self).__init__(config)
...
class GraspMetaForCausalLM(ABC):
    ...
    def encode_graphs(self, graph, graph_emb):
        ...
        graph_emb[graph == DEFAULT_GRAPH_PAD_ID] = 0.
        graph_features = self.get_model().mm_projector(graph_emb)
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if past_key_values is not None and graphs is not None and input_ids.shape[1] == 1:
            try:
                # Cache objects expose this API.
                past_len = past_key_values.get_seq_length()
            except AttributeError:
                # Tuple-of-tuples cache: take the K of the last layer.
                past_len = past_key_values[-1][-1].shape[-2]
            attention_mask = torch.ones((attention_mask.shape[0], past_len + 1),
